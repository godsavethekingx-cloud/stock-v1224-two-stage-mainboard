...
"""
import json, requests, time, re as _re, os
from collections import Counter
from concurrent.futures import ThreadPoolExecutor, as_completed

# 复用会话 + 代理支持 + 连接池(提升批量请求稳定性与并发效率)
_SESSION = None
_EW = 16   # 并发度
def _get_session():
    global _SESSION
    if _SESSION is None:
        ...
        s = requests.Session()
        s.headers.update(H)
        adapter = HTTPAdapter(pool_connections=24, pool_maxsize=24, max_retries=1)
        s.mount('http://', adapter)
        s.mount('https://', adapter)
        # 沙箱通过本地代理对外
        if 'HTTP_PROXY' in os.environ or 'http_proxy' in os.environ:
            s.proxies.update({'http': os.environ.get('HTTP_PROXY') or os.environ.get('http_proxy'),
...
def _get(url):
    try:
        return _get_session().get(url, timeout=10).text
    except Exception:
        return ''
...
def qt_bulk(codes):
    ...
    for i in range(0, len(codes), 60):
        ...
        try:
            t = _get_session().get('https://qt.gtimg.cn/q=' + q, timeout=10)
            raw = t.content.decode('gbk', 'ignore') if t.encoding == 'ISO-8859-1' else t.text
        except Exception:
            continue
...
def _pullback_probe(nm, code, today, kw_hit=True):
    """单只标的回踩判定(线程安全), 命中返回dict否则None"""
    try:
        ...
        try:
            t = _get_session().get(u, timeout=12).text
        except Exception:
            return None
        ...
        if len(ds) < 21 or ds[-1] != today: return None
        t1 = cc[-1]; t1v = vv[-1]
        if t1v <= 0: return None
        ma5 = sum(cc[-5:]) / 5; ma10 = sum(cc[-10:]) / 10; ma20 = sum(cc[-20:]) / 20
        if not (t1 > ma20 and t1 <= max(ma5, ma10) * 1.01): return None
        chg = (t1 / cc[-2] - 1) * 100
        if not (-6.0 <= chg <= 3.5): return None
        v5 = sum(vv[-6:-1]) / 5
        vr = t1v / v5 if v5 else 99
        if not (0.4 <= vr <= 2.2): return None
        ...
        return {'c': code, 'n': nm, 'pct': chg, 'vr': vr, 'sec': sec[:12], 'score': round(score, 2)}
    except Exception:
        return None
...
def live_pullback(topk=6):
    ...
    并发扫描系统板块映射成分股, 找"回踩MA5/MA10不破MA20+多头+缩量"的低吸票.
    返回: {'day':当前交易日,'rows':[{c,n,pct,chg,vr,sec,score}], 'count_check':愿检数}
    """
    global match_sector
    try:
        from backtest_v1224_1month import get_all_stocks as _gas, match_sector as _ms
        match_sector = _ms
    except Exception:
        from backtest_v1224_1month import get_all_stocks as _gas, match_sector as _ms
        match_sector = _ms
    ...
    cands = [s for s in stocks
             if (match_sector(s['name']) or any(kw in s['name'] for kw in EV_MAIN))]
    ncheck = 0
    with ThreadPoolExecutor(max_workers=_EW) as ex:
        futs = {ex.submit(_pullback_probe, s['name'], s['code'], today): s for s in cands}
