...
# 并发采集参数: 控制的并行度, 太大易触发限流, 太慢收益不明显
_MAX_WORKERS = 24          # 并发线程数
_BATCH = 40                # 每批取K线条数(下调以保证稳定性)
...
# 复用会话+轻量重试的并发请求器(线程安全)
# 连接池大小按并发度配置, 避免线程排队等待连接
from requests.adapters import HTTPAdapter
_SESSION = None
def _http_get(url, timeout=(6, 12), retries=2):
    global _SESSION
    if _SESSION is None:
        s = requests.Session()
        s.headers.update(HEADERS)
        pool = max(_MAX_WORKERS + 8, 32)
    ...
    for _ in range(retries):
        try:
            ...
            last = e
        time.sleep(0.05)
    raise last if last else Exception('http failed')
...
def get_all_stocks():
    ...
    print(f"  总代码:{len(codes)} 批次:{nb} 并发:{min(_MAX_WORKERS, nb)}", flush=True)

    done = 0
    with ThreadPoolExecutor(max_workers=min(_MAX_WORKERS, nb)) as ex:
        for res in as_completed([ex.submit(_parse_stock_batch, b) for b in batches]):
            batch_stocks = res.result()
...
def get_realtime_quote(codes):
    """批量获取实时行情(并发), 用于竞价强度和开盘价"""
    if not codes:
        return {}
    results = {}
    bs = 80
    batches = [codes[i:i+bs] for i in range(0, len(codes), bs)]
    with ThreadPoolExecutor(max_workers=min(_MAX_WORKERS, len(batches))) as ex:
        for res in as_completed([ex.submit(_parse_quote_batch, b) for b in batches]):
            try:
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=_MAX_WORKERS) as ex:
        futs = {ex.submit(get_kl, s['code'], 80): s for s in sampled}
        for fut in as_completed(futs):
            ...
            try:
                ...
                pass
            if done_n % _BATCH == 0 or done_n == total:
                el = time.time() - t0
                print(f"  K线进度:{done_n}/{total} 成功:{len(all_kls)} 用时:{el:.0f}s", flush=True)
    print(f"  K线获取成功: {len(all_kls)}只 (用时 {time.time()-t0:.0f}s)")
